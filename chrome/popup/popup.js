...
/*
	Download file in json format to the Chrome Downloads location
...
function populateInfo() {
	chrome.storage.local.get({ "name": "", "email": "", "myPrivateKey": "", "myPublicKey": "" }, function(result) {
	...
	chrome.storage.local.get({ passphrase: null }, function (result) {
		...
		if(result.passphrase == document.getElementById('pass-text').value) {
			...
			if(redirectOption) {
				...
				chrome.storage.local.remove(['name', 'privateKey', 'publicKey', 'passphrase'], function() {
					changeTab('user-none');
				});
			}
		} else
...
/*
	Get list of public keys from Chrome storage, either update selection boxes or download them
...
async function generateKey() {
	...
	chrome.storage.local.set({ name: nameVal, email: emailVal, myPrivateKey: privateKeyArmored, myPublicKey: publicKeyArmored, passphrase: passVal });
...
function decryptMessage() {
	chrome.storage.local.get({"myPublicKey": null, "myPrivateKey": null, "passphrase": null}, async function(result) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	document.getElementById('options-none').addEventListener('click', function () { chrome.runtime.openOptionsPage(); });
	document.getElementById('options-main').addEventListener('click', function () { chrome.runtime.openOptionsPage(); });
