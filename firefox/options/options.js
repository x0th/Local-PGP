...
function deleteParameters(option) {
	browser.storage.local.set({ "deleteStored": option });
}

function setContextMenus(option) {
	browser.storage.local.get({ "contextMenus": [true, false, false] }, function(result) {
		...
		browser.storage.local.set({ contextMenus: contextMenus });
	});
}

function defaultChecks() {
	browser.storage.local.get({ "deleteStored": 0, "contextMenus": [true, false, false] }, function(result) {
